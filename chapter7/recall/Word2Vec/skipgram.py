...
class SkipGram:
    ...
    def __init__(self,config):
        self.data_index = 0
        self.trainpath = config["train_file_data"]
        self.modelpath = config["sk_model_path"]
        self.min_count =  config["min_count"]#最低词频，保留模型中的词表
        self.batch_size = config["batch_size"] # 每次迭代训练选取的样本数目
        self.embedding_size =  config["embedding_size"]  # 生成词向量的维度
        self.window_size = config["skipgram_window_size"] # 考虑前后几个词，窗口大小, skipgram中的中心词-上下文pairs数目就是windowsize *2
        self.num_sampled = config["num_sampled"]  # 负样本采样.
